...
import com.joelkanyi.jcomposecountrycodepicker.resources.ic_arrow_drop_down
...
import com.joelkanyi.jcomposecountrycodepicker.utils.getDefaultCountryCode
 ...
 */
internal class KomposeCountryCodePickerDefaults {
    val selectedCountryCode: String = getDefaultCountryCode()
}
...
): CountryCodePicker = rememberSaveable(saver = CountryCodePickerImpl.Saver) {
    val countryCode =
        defaultCountryCode ?: KomposeCountryCodePickerDefaults().selectedCountryCode
    CountryCodePickerImpl(
 ...
 *
 * All lambda parameters ([onValueChange], [placeholder], [trailingIcon],
 * [countrySelectionDialogTitle], [countrySelectionDialogBackIcon],
 * [countrySelectionDialogSearchIcon]) are invoked synchronously on the
 * main thread during composition. Exceptions thrown inside these lambdas
 * will propagate to the caller and may crash the composition — callers
 ...
 * @param onValueChange Called on the main thread when the text field
 *    value changes. The new value is passed as a parameter. Exceptions
 *    thrown in this lambda will propagate to the caller.
 ...
 *    will be displayed without the text field.
 * @param shape The shape of the text field's outline.
 * @param placeholder A composable lambda invoked during composition to
 *    display the placeholder in the text field. Receives the current
 ...
 * @param countrySelectionDialogContainerColor The color of the country
 *    selection dialog container.
 * @param countrySelectionDialogContentColor The color of the country
 *    selection dialog content.
 * @param countrySelectionDialogTitle A composable lambda to display the
 *    title of the country selection dialog.
 * @param countrySelectionDialogBackIcon A composable lambda to display
...
public fun KomposeCountryCodePicker(
    ...
    if (showOnlyCountryCodePicker) {
        ...
        OutlinedTextField(
            ...
            colors = colors,
            isError = error,
            visualTransformation = remember(state.countryCode) {
                PhoneNumberTransformation(state.countryCode.uppercase())
            },
...
private fun DefaultPlaceholder(
    ...
    Text(
        ...
        text = stringResource(PickerUtils.getNumberHint(PickerUtils.allCountries.single { it.code == defaultLang }.code.lowercase())),
...
private fun SelectedCountryComponent(
    ...
    Row(
        ...
        if (showCountryName) {
            Text(
                text = stringResource(PickerUtils.getCountryName(selectedCountry.code.lowercase())),
                fontWeight = FontWeight.Bold,
                modifier = Modifier
                    .padding(start = 6.dp)
 ...
 */
internal fun Modifier.qaAutomationTestTag(tag: String): Modifier = this.then(
    Modifier.semantics {
        this.testTag = tag
    },
)
