...
internal actual fun getDefaultCountryCode(): String {
    ...
    try {
        val languages = window.navigator.asDynamic().languages as? Array<String>
        if (languages != null && languages.isNotEmpty()) {
            val region = TimezoneCountryMap.extractRegionFromLocales(languages.toList())
    ...
    try {
        val timezone = js("Intl.DateTimeFormat().resolvedOptions().timeZone") as? String
        if (timezone != null) {
...
internal actual fun String.normalizeUnicode(): String = this.asDynamic().normalize("NFD") as String
